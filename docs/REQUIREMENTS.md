...
| AGENT-18 | A model the provider refuses to serve us at all is disabled rather than paired again. Nothing in the catalogue predicts it — a gated model advertises tool support, a full window and 100% uptime — so it is learned from a refusal and remembered. | M |
