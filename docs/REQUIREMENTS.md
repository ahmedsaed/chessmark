...
| AGENT-21 | A model whose endpoint window cannot hold **one turn** is not paired. The check is possible before a game — endpoint context lengths are stored and the size of a turn is ours to measure — so it belongs beside the AGENT-14 registry rules rather than being discovered at ply 40. With it in place, a mid-game context failure means the model filled its own window, which forfeits it honestly. | M |
