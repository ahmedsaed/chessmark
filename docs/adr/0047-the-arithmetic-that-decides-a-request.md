# 0047. The arithmetic that decides whether a request can be sent

**Status:** Accepted
**Date:** 2026-09-06
**Renumbered** from 0032 on 2026-09-18. It was written as 0032 on the same day as *The leaderboard
is stored, not recomputed on every request*, which also took 0032 and which the index recorded;
this one was never indexed. Two decisions answering to one number meant a citation of "ADR-0032"
carried no information — and `CHANGELOG.md` had already resolved a reference to the wrong one. The
number changed; nothing about the decision did. `0032-the-arithmetic-that-decides-a-request.md`
remains as a pointer so existing links still land.
