...
| **`TOOL_SCHEMA_VERSION` is recorded but never checked.** `bench/ratable.GameFacts` does not carry it, so `judge` can exclude a game for its prompt version and not for the tool surface it played under — and a tool's *output* can change what the task is while both versions sit still, as `check`/`checkmate` did (ADR-0040). Harmless so far only because every tool change has moved `PROMPT_VERSION` alongside it. Closing it means deciding what a *minor* tool change is, which is ADR-0038's question about a second artefact. | Phase 12 |
