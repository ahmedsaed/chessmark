...
| AGENT-22 | A **read-only tool asked the same question three times in one turn is answered with a nudge**, not with the answer. The exchange is deterministic — identical messages in, identical result out — so repeating the answer is the one reply guaranteed to repeat the question, and one model spun `get_move_history` twenty times, spent 1.96M prompt tokens and twenty of the daily free allowance, and forfeited on `max_tool_iterations` without ever being told it was looping. The nudge names the tool, says nothing changes until it moves, and gives the tool rounds remaining. `make_move` is excluded: a repeated one is an illegal-move retry and belongs to AGENT-06. Games already lost this way stand (ADR-0026). | M |
| AGENT-23 | **Decision models play.** A model OpenRouter lists under `output_modalities=decisions` is registered as a `decision` runtime and seated like any model, but asked through the Decisions API: one request per turn carrying the position and every legal move, each described by facts code computes, and never a fact that looks past the move (ADR-0049). Its answer is validated against the moves offered before the referee sees it, and an unusable answer fails the turn without forfeiting anyone. | M |
| AGENT-24 | A decision seat is asked **everything a chat seat can do** on a turn, as one `choice` beside its move: play on, offer a draw with the move, resign, accept an open offer, or claim a draw when the referee would allow it. The action it ranks first happens, with no per-model threshold — except that resigning, accepting and claiming need **more than half** of its ranking, since they cannot be taken back; the record says when that rule overruled its first choice (ADR-0051). | M |
| AGENT-25 | The decision harness is versioned apart from the chat prompt and tools (`DECISION_VERSION`), a game records the version of each harness that played and only those, and a recorded request fails the suite when what a decision model is shown changes without a bump. | M |
| AGENT-26 | A decision model is **checked once before it is offered**: the catalogue refresh asks each one not yet checked under the current `DECISION_VERSION` one request in a turn's exact shape, and only a model that answered it is playable anywhere a model can be seated. A refusal is recorded with the host's own message; a rate limit or outage defers the check; nothing is re-asked on routine refreshes (ADR-0051). | M |
...
| BENCH-12 | A rating says **how sure it is** in a form a reader can act on. A new contestant starts at 1500 ± 500 — a wider prior than Glickman's 350, because the matchmaker deliberately pairs whoever is least known, so this population is mostly new and its first games should count for what they are worth. A rating above ± 110 is marked **provisional**; both numbers are Lichess's, adopted verbatim rather than tuned, and on today's data the mark applies to every contestant, which is the honest thing for the page to say. Provisional is a caveat and never reorders; *unrated* is a different statement and is not marked provisional (ADR-0028). | M |
| BENCH-13 | Decision models share the leaderboard and are **marked** wherever a model is named, because they play with every move described and cannot make an illegal one. Their tournaments seat decision models only, in an era of their own. A game is held to the version of each harness that played in it, so a chat-prompt bump never retires a decision game and a decision bump never retires a chat game (ADR-0049). | M |
| BENCH-14 | A pool may be given a **per-pair target**: each pair plays that many decided games per era and is then not paired again; a pool whose every pair has reached it is idle rather than finished, and wakes to play only a newcomer's pairs. Abandoned pairings do not count, an existing pool is unchanged until an operator sets one, and the page says when a pool is idle for this reason (ADR-0050). | M |
...
| OPS-21 | `status` answers **"is the harness healthy"**, which is a different question from "are the containers up" — a stack can be entirely running while every game is paused behind a rate limit, the free allowance is spent, and a pool has not moved a piece since yesterday. It reports the halt, both budgets, the queue, every turn that crashed in the last day (recorded by the worker, which survives the crash, and never written to the game's own log), every live and paused game with why and for how long, and each event's settled/abandoned split. Colour is a judgement: green working, amber worth a look, red needs somebody, with everything not green repeated at the end so "is anything wrong" never means reading every line. Every section is independently fault-tolerant, because a status command that dies on one broken datastore is useless exactly when it is needed. | S |
| OPS-22 | **A game in progress outranks a new one.** A pairing whose pause has expired reserves its concurrency slot, so the runner cannot fill it with a fresh pairing while the reconciler is waiting to put the old one back; and the patience window before a game is abandoned runs from its **last move**, not from its first pause. Without the first, 48–99% of every pause in `pool-free` was our own queue and one game waited 16.4 hours for a 60-second backoff; without the second, three games that had never gone 17.4 hours without moving were abandoned at plies 71, 68 and 56, each within seven playing-hours of a result (ADR-0025). | M |
| OPS-23 | **The model catalogue refreshes itself.** `refresh_catalogue.py` was written to be scheduled and nothing scheduled it, so the registry changed only when somebody remembered a command. Prices set the spend caps *and* what a user is charged in credits (ADR-0016) and endpoint rows are what the picker pins to, so a stale catalogue is a wrong cap, a wrong price and a field of models that cannot be played. The schedule is a service in the stack rather than a host timer, because a crontab lives nowhere in this repository and a rebuilt server loses it silently. A pass that cannot reach OpenRouter changes nothing and the loop waits for the next one — a scheduled sweep that exits on a bad night is restarted straight back into it. | S |
| OPS-24 | **The tournament runner discovers its events rather than being told one.** The long-running container ticked whichever slug `TOURNAMENT_SLUG` named, so creating a tournament through the CLI produced an event nothing would ever tick and abandoning one left the container ticking a corpse. Both failures are silent — the pool sits at zero pairings, indistinguishable from a matchmaker that cannot find a game — and both were fixed by editing `.env` and restarting a container, which is a deploy-shaped action for what `tournament create` already expresses. Which events exist is the database's business. Every unfinished one is ticked each pass, so two pools need one container and one event's failure does not take the others down with it. | S |
