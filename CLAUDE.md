...
| `GET /games/human-record` | **one statement, four counts.** The human-versus-model record, tallied with `FILTER` where the rows are rather than by reading the archive and counting in Python — the landing page calls it, and the Python version is a full scan per request |
