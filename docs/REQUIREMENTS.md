...
| AGENT-24 | A decision seat is asked **everything a chat seat can do** on a turn: resign and offer a draw every turn, accept an open offer, claim threefold or fifty-move when the referee would allow it. Code acts on the answers in a fixed order — claim, accept, resign, move with any offer — through gates set from a probe on labelled positions (`make probe-decisions`), one gate for every model. | M |
