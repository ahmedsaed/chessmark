...
| **Seven duplicate `game_ended` rows across four games, all of them written before ADR-0022's fix.** `29e7f004-2827-4621-b360-cf817c0c0a1d` (three: `timeout`→`timeout` 8m apart, `truncated`→`truncated` **50ms** apart, `truncated`→`abandoned`), `b5546c1b-0ced-4795-8488-6c93679f8a2f` (two), `0fbe4bae-ba7f-497f-8fc2-c72bfda95e0d` (`timeout`→`timeout`, 5m), `e601f9af-c7cf-4d29-99e3-e6597e4ceba4` (`truncated`→`truncated`, 17m). A game may legitimately end more than once — ended, resumed, ended again — so these are only the pairs with **no `game_resumed` between them**; six further multi-ending games in the same pool are ordinary resumes. The latest of the seven is `2026-08-30T05:03:59Z`, about sixteen hours before `e03327f` (ADR-0022, `2026-08-30T21:11Z`), and a sweep of all 48 `pool-free` games found **none after it** — so the evidence says fixed, not quiet. What is undecided is the residue: a duplicate ending may have overwritten an earlier verdict, and `resume --overwritten-verdict` reopens on the *first* ending where that first was a harness stop. Nobody has checked these four against that, and ply counts on games from before the fix are not fully trusted. | Phase 5 |
