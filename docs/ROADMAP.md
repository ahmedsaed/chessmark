...
| **A signed-in browser test fails and did so before this work.** `the model's thinking is hidden while the game is live` times out waiting for a folded turn to expand. Reproduced on `main` at `bbd5756` as well as on the branch that found it, so it is not a regression from the auth or Clerk-scoping changes — it is either a race with the scripted worker or a real `EventStream` fault, and nobody has looked. It is the only red in `make test-e2e-all`. | Phase 23 |
