...
| **Clerk's core is still 87 KiB on routes that need no identity.** Its prebuilt UI is gone — owning the auth screens let `prefetchUI={false}` apply, taking Clerk from 372 KiB to 87 and the page from 619 to 337. What remains is `clerk-js` itself, loaded because `AccountBar` calls `useAuth()` in the global header. Mounting `ClerkProvider` only when a route needs it, or when the `__client_uat` cookie shows a session, would take a signed-out reader to zero. Not done: it is a change to the auth surface and wants the signed-in browser suite first. | Phase 7 |
