...
| **Clerk is 370 KiB — 55% of a production page — on every route, including ones that need no identity.** Its prebuilt UI is 285 KiB of that; the core is 87 KiB. `prefetchUI={false}` on `ClerkProvider` drops it to 87 KiB and takes the page from 619 to 334 KiB — **measured, not estimated** — but applied globally it breaks `/sign-in`, which needs the UI bundle it just stopped prefetching. Capturing the saving means scoping the flag to the routes that render no Clerk UI, which is real work and has not been done. It is why production scores 87 where a Clerk-free build scores 99. | Phase 7 |
