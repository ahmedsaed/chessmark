...
| **`PROMPT_VERSION` was not bumped for the closing round** ([ADR-0037](adr/0037-a-turn-ends-when-the-model-stops.md)). A turn now ends when the model stops rather than when it moves, so every model gets a round it did not have and its own prose enters the transcript that later turns read. Games before and after are not strictly the same task. Not bumped because a bump resets the leaderboard's comparability and there are other prompt changes under consideration — the colour in the turn prompt, the FEN — which should reset it once between them rather than three times. | Phase 5 |
...
| **`get_move_history` and `make_move` keep the `+` and `#` that `get_legal_moves` strips.** `tools.py` sends the legal-move list through `plain_san` (ADR-0042) and calls `history_san()` directly, so a seat reading its own history sees `Qh1+`, `Qxd1+`, `Qd2+`. It is not the v3 leak — these are moves already played and it hands over no search — but it is the information class ADR-0040 removed the `check` flag for, since a model hunting a repetition can read which of its own moves gave check instead of deriving them. Left alone deliberately: stripping it is two lines, and the cost is a `TOOL_SCHEMA_VERSION` bump that resets the current era. To be decided, not drifted into. | Phase 5 |
